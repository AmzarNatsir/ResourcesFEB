      ...
      <div class="col-xs-12">
        <div class="box">
          <div class="box-header">
            <h3 class="box-title">DAFTAR AKUN DOSEN (AKTIF)</h3>
            <hr>
            <div class="box-tools">
          ...
          <div class="box-body table-responsive">
            <table class="table tabel_list" width="100%">
              ...
              <tbody>
                <?php $nom=1; foreach ($dosen_all_aktif as $list_aktif): ?>
                  <tr>
                    <td><?= $nom; ?></td>
                    <td><?= $list_aktif['nidn']; ?></td>
                    <td><?= $list_aktif['nama_dosen']; ?></td>
                    <td><?= $list_aktif['email']; ?></td>
                    <td><?= date_format(date_create($list_aktif['tgl_act']), 'd-m-Y'); ?></td>
                    <td><button type="button" class="btn btn-warning edit_form" data-toggle="modal" data-target="#modal-edit" title="Edit Password Dosen" id="<?= $list_aktif['id'];?>"><i class="fa fa-key"></i> Rubah Password</button></td>
                  </tr>
                <?php $nom++; endforeach ?>
              </tbody>
            </table>
...
<script type="text/javascript">
  $(document).ready(function() 
  {
    ...
    {
      $("#frm_modal").load("<?php echo base_url();?>home/add_akun_dosen");
    });
    $(".tabel_list").on("click", ".edit_form", function()
    {
      var id_data = this.id;
      $("#frm_modal_edit").load("<?php echo base_url();?>home/edit_akun_dosen/"+id_data);
    });
  });
</script>
